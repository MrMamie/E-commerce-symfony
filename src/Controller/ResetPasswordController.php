...
class ResetPasswordController extends AbstractController
{
    ...
    public function index(Request $request): Response
    {
        ...
        if ($request->get('email')) {
            ...
            if ($user) {
                ...
                $content =  'Bonjour, ' . $user->getFirstname() . "<br> Vous avez demandé à réinitialiser votre mot de passe sur le site Art2pix <br><br>";
                $content .= "Merci de bien vouloir cliquer sur le lien suivant <a href='" . $url . "'>mettre à jour votre mot de passe</a>. ";
                $mail->send($user->getEmail(), $user->getFirstName() . ' ' . $user->getLastname(), 'Réinitialiser votre mon de passe', $content);
    ...
    public function update(Request $request, $token,UserPasswordHasherInterface $encoder)
    {
        ...
        if($form->isSubmitted() && $form->isValid()){
            ...
            $reset_password->getUser()->setPassword($password); // sa fonctionne malgré l'erreur
            $this->entityManager->flush();
            $this->addFlash('notice', "Votre mot de passe à bien été mis a jour.");
